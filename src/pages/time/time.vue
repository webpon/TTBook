<template>
	<view class="flex items-center flex-col" v-if="hasUserName">
		...
		<view class="flex my-20px w-270px">
			<button
				class="-indent-3px leading-36px w-37px h-37px rounded-37px flex-shrink-0 border-1rpx border-solid border-gray-500"
				@click="simpleChangeDay(-1)">{{ '<' }}</button>
					<view class="px-20px">
						<uni-datetime-picker type="date" :clear-icon="false" v-model="currentDay" @change="changeDay" />
					</view>
					<button
						class="-indent-2px leading-36px w-37px h-37px rounded-37px flex-shrink-0 border-1rpx border-solid border-gray-500"
						@click="simpleChangeDay(1)">></button>
		</view>

		<view class="w-90vw bg-blue-300 rounded-20rpx">
			<uni-steps v-if="orderList.length > 0" @clickItem="editRecord" @longpressItem="delRecord"
				active-color="skyblue" deactiveColor="purple" direction="column" :active="-1" :options="orderList"
...
function simpleChangeDay(flag) {
	let current = new Date(currentDay.value).getTime()
	if (flag === -1) {
		current = current - 86400000
	} else if (flag === 1) {
		current = current + 86400000
	}
	currentDay.value = getDate(new Date(current)).fullDate
	getTimeRecord()
}
...
function dialogInputConfirm(title: string) {
	if (title.length !== 0) {
		...
		const startTimeDesc = getDate(new Date()).simpleTime
		const overTimeDesc = ''
		uni.request({
			method: "post",
...
function delRecord(item: any) {
	uni.showActionSheet({
		itemColor: "#FF0000",
		itemList: item.overTime ? ["删除该记录", "修改记录"] : ["删除该记录"],
		success: function (res) {
			...
			} else if (tapIndex === 1 && item.overTime) {
				item.editFlag = true
				uni.navigateTo(addUrlParams(
					"/pages/time/subPage/recordEdit",
					item
